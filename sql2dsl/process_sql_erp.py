...
def parse_sql_dsl(sql, date_cols, dsl):
    ...
    for i, d in enumerate(dsl):
        ...
        if i == 0:
            ...
            # dls_in后第一dsl的input为join之后的output，不是dsl_in后的output
            if from_length > 0:
...
def datesub_to_time_now(toks):
    ...
    if "datesub" in toks_str or "date_sub" in toks_str:
        for i, tok in enumerate(toks):
            if tok == "date_sub(time_now()" or tok == "date_sub(current_date()" or tok=="date_sub(now()":
                datesub_index = i
            if tok == "interval":
                interval_index = i
...
def get_sql(schema, query, table_info, date_cols, question=""):
    ...
    toks, _ = datesub_to_time_now(toks)
